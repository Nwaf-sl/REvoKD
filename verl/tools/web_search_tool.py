...
from verl.tools.utils.search_r1_like_utils_api import perform_single_search_batch
...
class SearchTool(BaseTool):
    ...
    def execute_search(self, instance_id: str, query_list: list, retrieval_service_url: str, topk: int, timeout: int):
        ...
        # print("DEBUG result_text type:", type(result_text))
        # print("DEBUG metadata type:", type(metadata))
        # print("DEBUG metadata contents:", metadata)
        # 调试一下返回值是否可序列化
        # try:
        #     pickle.dumps(result_text)
        #     pickle.dumps(metadata)
        # except Exception as e:
        #     logger.error(f"Pickle error in execute_search: {e}")
        #     raise

        # logger.debug(f"Return result_text type: {type(result_text)}, metadata type: {type(metadata)}")
    ...
    async def execute(self, instance_id: str, parameters: dict[str, Any], **kwargs) -> tuple[ToolResponse, float, dict]:
        ...
        try:
            ...
            print("DEBUG result:",result_text)
